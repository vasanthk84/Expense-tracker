 ...
 * Fixes: adds "Other" slice for remainder so slices always sum to total,
 *        legend shows $ + %, proper draw animation, tap-to-highlight.
 */
...
export default function DonutChart({ slices = [], total = '' }) {
  ...
  // Slices are now dollar amounts — compute % from them directly
  const sum = slices.reduce((acc, s) => acc + s.value, 0) || 1;

  // Largest-remainder so percentages always sum to exactly 100
  ...
  const pcts     = floored
    ...
    .sort((a, b) => b.r - a.r)
    .reduce((arr, { i, p }) => { arr[i] = p + (leftover-- > 0 ? 1 : 0); return arr; }, []);
