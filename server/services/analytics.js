 ...
 */
import { readTransactions, readJSON } from './storage.js';
...
function daysRemaining(yyyymm, today = new Date()) {
  ...
  if (!isCurrentMonth) {
    // Past or future month — return 0 for past, full days for future
...
/**
 * Sum spending (excludes income) for a given txn list.
 */
...
/**
 * Group spending by category for a txn list.
 * Returns { [categoryId]: total }
 */
...
/**
 * Monthly summary: spent, budget, remaining, days left.
 */
...
/**
 * Budget vs actual per category for a given month.
 */
...
/**
 * Donut breakdown by category — percentages for a date range.
 */
export async function categoryBreakdown({ from, to }) {
  ...
  // Standard color palette for slices
  const colors = ['var(--primary)', '#4C6B5D', 'var(--accent)', '#C9B48A', 'var(--border-strong)'];

  // Sort by value desc, top 4, group remainder as "Other"
...
/**
 * Trend: monthly totals for the last N months (ending at given month).
 */
export async function monthlyTrend(endMonth, count = 6) {
  const months = [];
  for (let i = count - 1; i >= 0; i--) {
    months.push(offsetMonth(endMonth, -i));
  }
...
/**
 * Comparison: this month vs last month per top categories.
 */
export async function categoryComparison(yyyymm) {
  ...
  // Take top 5 categories by current-month spending
...
/**
 * Forecast: simple linear projection of next N months based on past N months.
 * Uses average of past months with mild trend extrapolation.
 */
export async function forecast(endMonth, pastMonths = 6, futureMonths = 6) {
  ...
  if (!values.length) {
    return { actual: trend, forecast: [], upper: [], lower: [], projected: 0, yoyChange: 0 };
  }
  ...
  // Forecast points: start at last actual, then project forward
  ...
  // YoY change vs last year same period (simple: vs avg)
...
/**
 * Insights: programmatic pattern detection.
 * Returns up to N suggestion objects.
 */
export async function insights(yyyymm) {
  ...
  // 1. Categories that increased >15% MoM
  ...
  // 3. Day-of-week pattern: highest spend day
...
/**
 * Day-of-week spending pattern (for the insights screen viz).
 */
export async function weeklyPattern(yyyymm) {
  ...
  // Reorder Mon-first for display: Mon Tue Wed Thu Fri Sat Sun
  ...
  return {
    ...
    topAvg: Math.round(totals[topIdx] / 4) // ~4 weeks per month
  };
}
